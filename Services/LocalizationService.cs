...
{
    class MessageLocalizationFile
    {
        public Dictionary<string, string> Messages { get; set; }
    }

    static readonly Dictionary<string, string> _localizedMessages = [];

    static readonly Dictionary<string, string> _messageLocalizationFiles = new()
    {
        {"English", "Bloodcraft.Resources.Localization.Messages.English.json"},
        {"TraditionalChinese", "Bloodcraft.Resources.Localization.MessagesTChinese.json"},
        {"SimplifiedChinese", "Bloodcraft.Resources.Localization.MessagesSChinese.json"}
    };
    ...
    public static void HandleReply(ChatCommandContext ctx, string message)
    {
        ...
        if (_localizedMessages.TryGetValue(message, out var localized))
        {
            ctx.Reply(localized);
        }
        else
    ...
    */
    ...
    {
        if (!_messageLocalizationFiles.TryGetValue(_language, out var resourceName))
        {
            Plugin.LogInstance.LogWarning($"[Localization] Message localization file not found: {resourceName}");
            return;
        }
        ...
        var file = JsonSerializer.Deserialize<MessageLocalizationFile>(json, new JsonSerializerOptions
        {
            PropertyNameCaseInsensitive = true
        });
        ...
        {
            Plugin.LogInstance.LogWarning("[Localization] Message localization file loaded but Messages is null.");
            return;
        }

        foreach (var kv in file.Messages)
        {
            _localizedMessages[kv.Key] = kv.Value;
        }

        Plugin.LogInstance.LogInfo($"[Localization] Loaded {_localizedMessages.Count} localized messages.");
    }
