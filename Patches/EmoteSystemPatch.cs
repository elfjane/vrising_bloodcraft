...
internal static class EmoteSystemPatch
{
    ...
    public static void BindUnbind(User user, Entity playerCharacter, ulong steamId)
    {
        ...
        else
        {
            ...
            {
                LocalizationService.HandleServerReply(EntityManager, user, "You can't summon your familiar during combat!");
                return;
            }
    ...
    public static void CallDismiss(User user, Entity playerCharacter, ulong steamId)
    {
        ...
        if (steamId.HasDismissedFamiliar())
        {
            if (playerCharacter.PlayerInCombat())
            {
                LocalizationService.HandleServerReply(EntityManager, user, "You can't call your familiar during combat!");
                return;
            }
            ...
            var dismissed = actives.Where(a => a.Dismissed).ToList();
            foreach (var a in dismissed)
            {
                CallFamiliar(playerCharacter, a.Familiar, user, steamId);
            }

            // ensure AutoCallMap points at the most recently summoned (if any)
            var last = actives.LastOrDefault();
            if (last != null && last.Familiar.Exists())
            {
                AutoCallMap[playerCharacter] = last.Familiar;
            }
        ...
        if (actives.Any(a => a.Familiar.HasBuff(_interactModeBuff)))
        {
            ...
            {
                DismissFamiliar(playerCharacter, a.Familiar, user, steamId);
            }
        ...
        var lastActive = actives.LastOrDefault();
        if (lastActive != null && lastActive.Familiar.Exists())
        {
            AutoCallMap[playerCharacter] = lastActive.Familiar;
        }
    }
    public static void CombatMode(User user, Entity playerCharacter, ulong steamId)
    {
        ...
        else if (steamId.HasActiveFamiliar())
        {
            ...
            bool anyInvul = actives.Any(a => a.Familiar.HasBuff(_invulnerableBuff));
            ...
            if (anyInvul)
            {
                ...
                {
                    var fam = a.Familiar;
                    fam.TryRemoveBuff(buffPrefabGuid: _invulnerableBuff);
                    fam.SetFaction(_playerFaction);
                    EnableAggro(fam);
                    Familiars.EnableAggroable(fam);
                }
            ...
            else
            {
                ...
                {
                    var fam = a.Familiar;
                    fam.TryApplyBuff(_invulnerableBuff);
                    fam.SetFaction(_ignoredFaction);
                    DisableAggro(fam);
                    Familiars.DisableAggroable(fam);
                }
    ...
    public static void InteractMode(User user, Entity playerCharacter, ulong steamId)
    {
        ...
        foreach (var a in actives)
        {
            var familiar = a.Familiar;
            var servant = a.Servant;
            var coffin = GetServantCoffin(servant);

            if (!familiar.Exists() || !servant.Exists() || !coffin.Exists()) continue;
            ...
            if (hasInteractBuff)
            {
                SaveFamiliarEquipment(steamId, a.FamiliarId, GetFamiliarEquipment(a.Servant));
