...
internal static class BuffSystemSpawnPatches
{
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                switch (buffType)
                {
                    ...
                    case 3 when isPlayerTarget: // Familiar and player has PvE Combat Buff
                        ...
                        {
                            Familiars.HandlePlayerEnteringCombat(buffTarget);
                        }
                        break;
                    case 4 when _familiars && isPlayerTarget: // Familiars and player has PvP Combat Buff
                        if (steamId.HasActiveFamiliar())
                        {
                            if (!_familiarPvP)
                            {
                                ...
                                {
                                    Familiars.DismissFamiliar(buffTarget, a.Familiar, combatUser, steamId);
                                }
                            }
                            else
                            {
                    ...
                    case 9 when _familiars:
                        ...
                        {
                            var fams = Familiars.ActiveFamiliarManager.GetActiveFamiliars(playerCharacter.GetUser().PlatformId);
                            if (fams != null)
                            {
                                foreach (var a in fams)
                                {
                                    var fam = a.Familiar;
                                    if (fam.Exists() && fam.TryGetBuff(_highlordGroundSwordBossBuff, out buffEntity))
                                    {
    ...
    static void HandlePveCombatBuff(BuffSpawnContext buff)
    {
        ...
        {
            Familiars.HandlePlayerEnteringCombat(buff.BuffTarget);
        }
    }
    static void HandlePvpCombatBuff(BuffSpawnContext buff)
    {
        ...
        if (buff.SteamId.HasActiveFamiliar())
        {
            if (!_familiarPvP)
            {
                var actives = Familiars.ActiveFamiliarManager.GetActiveFamiliars(buff.SteamId).Where(x => x.Familiar.Exists()).ToList();
                User user = buff.BuffTarget.GetUser();
                foreach (var a in actives)
                {
                    Familiars.DismissFamiliar(buff.BuffTarget, a.Familiar, user, buff.SteamId);
                }
            }
            else
            {
