...
internal static class DeathEventListenerSystemPatch
{
        ...
        ref ComponentLookup<VBloodConsumeSource> vBloodConsumeSourceLookup)
    {
        ...
        else if (_familiars && deathEvent.Died.TryGetFollowedPlayer(out Entity playerCharacter))
        {
            ...
            if (hasActive)
            {
                // Remove the specific familiar entry if it exists in the active list (handles multi-familiar scenarios)
                var actives = ActiveFamiliarManager.GetActiveFamiliars(steamId);
                if (actives != null)
                {
                    var match = actives.Find(x => x.Familiar == deathEvent.Died || x.FamiliarId == deathEvent.Died.GetGuidHash());
                    if (match != null)
                    {
                        // clean up servant (if present) and remove only the dead familiar from active list
                        Entity familiarServant = GetFamiliarServant(playerCharacter);
                        if (familiarServant.Exists()) DestroyFamiliarServant(familiarServant);

                        ActiveFamiliarManager.RemoveActiveFamiliar(steamId, deathEvent.Died);

                        // if there are no more active familiars, reset any remaining binding state
                        var remaining = ActiveFamiliarManager.GetActiveFamiliars(steamId);
                        if (remaining == null || !remaining.Any(x => x.Familiar.Exists()))
                        {
