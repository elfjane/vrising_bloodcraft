...
internal static class PrestigeCommands
{
    ...
    [Command(name: "self", shortHand: "buff", adminOnly: true, usage: ".prestige buff buffid", description: "法術增益測試指令")]
    public static void BuffPrestigeCommand(ChatCommandContext ctx, string prestigeType)
    {
        if (!ConfigService.PrestigeSystem)
        {
            LocalizationService.HandleReply(ctx, "Prestiging is not enabled.");
        ...
        if (!int.TryParse(prestigeType, out int number))
        {
            LocalizationService.HandleReply(ctx, $"[buff] fail");
            return;
        }
        Entity character = ctx.Event.SenderCharacterEntity;
        PrefabGUID buff = new(number);
        string buffName = buff.GetLocalizedName();
        ApplyPrestigeBuffs2(character, buff);
        LocalizationService.HandleReply(ctx, $"[buff] {prestigeType}, {buffName}");
    }

    [Command(name: "self", shortHand: "buffr", adminOnly: true, usage: ".prestige buff remove buffid", description: "法術增益移除測試指令")]
    public static void BuffRemovePrestigeCommand(ChatCommandContext ctx, string prestigeType)
    {
        if (!ConfigService.PrestigeSystem)
        {
            LocalizationService.HandleReply(ctx, "Prestiging is not enabled.");
        ...
        if (!int.TryParse(prestigeType, out int number))
        {
            LocalizationService.HandleReply(ctx, $"[buff remove] fail");
            return;
        }
        ...
        PrefabGUID buff = new(number);
        string buffName = buff.GetLocalizedName();
        character.TryRemoveBuff(buff);
        LocalizationService.HandleReply(ctx, $"[buff remove] {prestigeType}, {buffName}");
    }
}
