...
internal static class DataService
{
    ...
    // Per-player settings saved as key -> int values (e.g., MaxActiveFamiliars)
    public static class PlayerSettingsManager
    {
        static string GetFilePath(ulong steamId) => Path.Combine(DirectoryPaths[9], $"{steamId}_player_settings.json");
        public static void SavePlayerSettings(ulong steamId, Dictionary<string, int> settings)
        {
            string filePath = GetFilePath(steamId);
            Directory.CreateDirectory(Path.GetDirectoryName(filePath));
        ...
        public static Dictionary<string, int> LoadPlayerSettings(ulong steamId)
        {
            string filePath = GetFilePath(steamId);
            ...
            string jsonString = File.ReadAllText(filePath);
            return JsonSerializer.Deserialize<Dictionary<string, int>>(jsonString) ?? new Dictionary<string, int>();
        }
        public static Dictionary<string, int> GetOrInitializePlayerSettings(ulong steamId, Dictionary<string, int> defaultSettings)
        {
            var settings = LoadPlayerSettings(steamId);
            ...
            SavePlayerSettings(steamId, settings);
            return settings;
        }
    }
