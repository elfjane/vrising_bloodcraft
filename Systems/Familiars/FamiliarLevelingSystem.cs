...
internal static class FamiliarLevelingSystem
{
    ...
    static readonly bool _divideXpByActiveFamiliars = ConfigService.DivideFamiliarExperienceByActiveFamiliars;
    ...
    {
        // support multiple familiars: give XP to each active familiar that is eligible for combat
        var actives = Utilities.Familiars.ActiveFamiliarManager.GetActiveFamiliars(steamId);
        if (actives == null || !actives.Any()) return;

        var eligible = actives.Where(x => x.Familiar.Exists() && x.Familiar.EligibleForCombat()).ToList();
        ...
        foreach (var a in eligible)
        {
            ...
            ProcessExperienceGain(source, familiar, target, steamId, familiarId.GuidHash, groupMultiplier, eligibleCount);
        }
    }
    static void ProcessExperienceGain(Entity player, Entity familiar, Entity target, ulong steamId, int famKey, float groupMultiplier, int eligibleCount)
    {
        ...
        // Optionally divide XP across active familiars if configured to do so
        if (_divideXpByActiveFamiliars && eligibleCount > 1)
        {
            gainedXP /= eligibleCount;
        }
